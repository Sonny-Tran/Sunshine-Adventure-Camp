...
window_height = int(screen_height / 1.5) #Deciding window height based on screen size, so no matter what display, window will always take up 66.667%
window_width = int(screen_width / 3) #Deciding window width based on screen size, so no matter what display, window will always take up 33.333%

RATIO = 1
RATIO *= window_width / 640 # Since I'm programming on a 1920 x 1080 resolution screen, I use Ratio so that if some one else views this program with a 4k screen (3840 x 2160),
...
root.grid_rowconfigure(0, weight=1)  #Configuring weight of row '0', that means that if there's extra space, anything placed in row 0 will take up all that extra space
...
entry_frame.grid(row=1, column=0, sticky="ew", columnspan=2)
